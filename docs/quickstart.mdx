...
<Frame>
  <img src="graphics/ny_datasets.png" alt="michelin star restaurants in new york city" title="michelin star restaurants in new york city" />
</Frame>
...
def process_restaurant(restaurant, boroughs):
	# plot the primary table element
	plot(restaurant, 'restaurant')

	# find the intersecting boroughs
	intersecting_boroughs = boroughs.intersects(restaurant)
	plot(intersecting_boroughs, 'boroughs')
	...
	return {
		'borough': intersecting_boroughs[0]['boro_name']
	}
```
